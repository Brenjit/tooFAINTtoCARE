...
target_redshift_min = 1
target_redshift_max = 5
...
def create_zout_catalog(eazy_data, selected_sources, pointing, output_dir):
    ...
    with open(output_file, 'w') as f:
        f.write('# EAZY zout values for high-z sources (z=1-5)\n')
...
def create_sextractor_like_catalog(sextractor_data, selected_sources, pointing, output_dir):
    ...
    if output_data:
        ...
        with open(output_file, 'w') as f:
            f.write('# SExtractor-like catalog for 50 high-z sources (z=1-5)\n')
...
def create_individual_filter_catalogs(sextractor_data, selected_sources, pointing, output_dir):
    ...
    for filt in filters:
        ...
        if output_data:
            ...
            with open(output_file, 'w') as f:
                f.write(f'# SExtractor catalog for 50 high-z sources (z=1-5) - Filter: {filt}\n')
...
def main():
    """Main function to create catalogs for 50 high-z sources (z=1-5) per pointing."""
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"Creating catalogs for 50 high-z sources (z=1-5) per pointing")
    ...
    for pointing in pointings:
        ...
        # Select sources in redshift range 1-5
        ...
        print(f"Selected {len(selected_sources)} high-z sources (z=1-5)")
    ...
    if all_catalogs:
        ...
        if all_zout_catalogs:
            ...
            with open(master_zout_file, 'w') as f:
                f.write('# Master EAZY zout catalog for all high-z sources (z=1-5)\n')
        ...
        print(f"Total high-z sources (z=1-5) in master catalog: {len(all_data)}")
