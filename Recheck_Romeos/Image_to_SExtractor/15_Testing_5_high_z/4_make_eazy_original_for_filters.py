...
def get_optimal_process_count():
    ...
    if load_avg > logical_cores * 0.6:
        optimal = int(optimal * 0.7)
    
    # Adjust for low available memory (< 4GB)
    if memory.available < 4 * 1024 * 1024 * 1024:
        optimal = min(optimal, 2)
    
    # Safety limit: never exceed total logical cores - 1
    optimal = min(optimal, logical_cores - 1)
    ...
    if perform_empty_aperture == 'yes':
        ...
        for idx, row in df.iterrows():
            ...
            if generate_cutouts == 'yes':
                ...
                output_dir = "/media/iit-t/MY_SSD_1TB/Work_PhD/July-August/CEERS_data/EAZY/eazy-photoz/inputs/Results_15_Oct_40_threads/cutout_nmad"
...
def process_dropouts(dropout_type='z~12', pointing='nircam1', dr_version='dr0.5'):
    print(f"Processing for {dropout_type} dropout in {pointing} with {dr_version}")
    
    # Updated directory paths
    base_image_dir = f"/media/iit-t/MY_SSD_1TB/Work_PhD/July-August/CEERS_data/Romeo_s_data/{pointing}"
    base_catalog_dir = f"/home/iit-t/Bren_jit/tooFAINTtoCARE/Recheck_Romeos/Image_to_SExtractor/15_Testing_4_different_Z/50_2_random_sources_catalogues/individual_filters/{pointing}"
    ...
    output_dir = "/home/iit-t/Bren_jit/tooFAINTtoCARE/Recheck_Romeos/Image_to_SExtractor/15_Testing_4_different_Z/4_Eazy_catalogue_of_sept_data"
    
    os.makedirs(output_dir, exist_ok=True)
    
    output_file = os.path.join(output_dir, f"{pointing}_output.txt")
    fluxerr_catalogue_file = os.path.join(output_dir, f"{pointing}_fluxerr_catalogue.cat")
    eazy_catalogue_file = os.path.join(output_dir, f"{pointing}_eazy_catalogue.cat")
    
    catalog_files = {  
        "F606W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f606w_catalog.cat"),
        ...
        "F814W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f814w_catalog.cat"),
        ...
        "F115W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f115w_catalog.cat"),
        ...
        "F150W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f150w_catalog.cat"),
        ...
        "F200W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f200w_catalog.cat"),
        ...
        "F277W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f277w_catalog.cat"),
        ...
        "F356W": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f356w_catalog.cat"),
        ...
        "F410M": {
            "catalog": os.path.join(base_catalog_dir, f"random_50_f410m_catalog.cat"),
    ...
    num_processes = get_input_with_timeout(process_prompt, 5, str(optimal_processes))
    try:
...
def process_all_pointings():
    ...
    for pointing_num in range(1,11):
        ...
        print(f"\n{'='*50}")
        print(f"Processing {pointing} with {dr_version}")
        print(f"{'='*50}\n")
        
        try:
            process_dropouts(dropout_type=dropout_type, pointing=pointing, dr_version=dr_version)
            print(f"Completed processing for {pointing} with {dr_version}")
        except Exception as e:
            print(f"Error processing {pointing} with {dr_version}: {e}")
            continue

if __name__ == "__main__":
    process_all_pointings()
