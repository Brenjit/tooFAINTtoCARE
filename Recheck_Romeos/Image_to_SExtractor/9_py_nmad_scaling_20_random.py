...
# Number of random sources to select
NUM_RANDOM_SOURCES = 100
...
def process_single_filter(filter_name, paths, df, perform_empty_aperture, generate_cutouts, hst_photflam_lamda, aperture_corrections, pointing):
    print(f"Processing filter: {filter_name}")
    results = {}
    
    # Randomly select NUM_RANDOM_SOURCES from the catalog
    if len(df) > NUM_RANDOM_SOURCES:
        random_indices = random.sample(range(len(df)), NUM_RANDOM_SOURCES)
        df = df.iloc[random_indices].copy()
        print(f"Randomly selected {NUM_RANDOM_SOURCES} sources for {pointing}")
    else:
        print(f"Catalog has only {len(df)} sources, using all of them for {pointing}")
    ...
    if perform_empty_aperture == 'yes':
        ...
        for idx, row in df.iterrows():
            ...
            if generate_cutouts == 'yes':
                ...
                output_dir = f"/Volumes/MY_SSD_1TB/Work_PhD/July-August/CEERS_data/EAZY/eazy-photoz/cutout_nmad_20_random/{pointing}"
...
def process_dropouts(dropout_type='z~12', pointing='nircam1', dr_version='dr0.5'):
    ...
    output_dir = f"./9_01_nmad_20_random/{pointing}"
    
    os.makedirs(output_dir, exist_ok=True)
    
    output_file = os.path.join(output_dir, f"{pointing}_output.txt")
    fluxerr_catalogue_file = os.path.join(output_dir, f"{pointing}_fluxerr_catalogue_20_random.cat")
    eazy_catalogue_file = os.path.join(output_dir, f"{pointing}_eazy_catalogue_20_random.cat")
    ...
    for filter_name, paths in catalog_files.items():
        ...
        parallel_args.append((filter_name, paths, df, perform_empty_aperture, generate_cutouts, 
                            hst_photflam_lamda, aperture_corrections, pointing))
