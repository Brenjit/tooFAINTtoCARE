...
# Scaling factors for each pointing and filter
# Updated scaling factors for each pointing and filter
scaling_factors = {
    'nircam1': {
        'f606w': 0.69623, 'f814w': 0.33166, 'f115w': 0.71782, 'f150w': 0.51542,
        'f200w': 0.44576, 'f277w': 0.65191, 'f356w': 0.83187, 'f410m': 0.65723, 'f444w': 0.61749
    },
    'nircam2': {
        'f606w': 0.77297, 'f814w': 0.67423, 'f115w': 0.35926, 'f150w': 0.33716,
        'f200w': 0.35920, 'f277w': 0.47949, 'f356w': 0.53669, 'f410m': 0.73322, 'f444w': 0.73174
    },
    'nircam3': {
        'f606w': 1,       'f814w': 0.60898, 'f115w': 0.50857, 'f150w': 0.41571,
        'f200w': 0.49174, 'f277w': 0.45225, 'f356w': 0.51592, 'f410m': 0.88661, 'f444w': 0.84318
    },
    'nircam4': {
        'f606w': 0.73375, 'f814w': 0.39766, 'f115w': 0.42393, 'f150w': 0.34986,
        'f200w': 0.46364, 'f277w': 0.44272, 'f356w': 0.56959, 'f410m': 0.69602, 'f444w': 0.63416
    },
    'nircam5': {
        'f606w': 1,       'f814w': 0.72523, 'f115w': 0.76340, 'f150w': 0.00202,
        'f200w': 0.00156, 'f277w': 0.00121, 'f356w': 0.71814, 'f410m': 0.00095, 'f444w': 0.00065
    },
    'nircam6': {
        'f606w': 1,       'f814w': 1,       'f115w': 0.45913, 'f150w': 0.44211,
        'f200w': 0.39994, 'f277w': 0.46962, 'f356w': 0.59112, 'f410m': 0.53249, 'f444w': 0.92619
    },
    'nircam7': {
        'f606w': 0.50932, 'f814w': 0.48553, 'f115w': 0.43789, 'f150w': 0.00148,
        'f200w': 0.00108, 'f277w': 0.00112, 'f356w': 0.55363, 'f410m': 0.00101, 'f444w': 0.00083
    },
    'nircam8': {
        'f606w': 0.35931, 'f814w': 0.43831, 'f115w': 0.57011, 'f150w': 0.00152,
        'f200w': 0.00146, 'f277w': 0.00104, 'f356w': 0.50656, 'f410m': 0.00092, 'f444w': 0.00057
    },
    'nircam9': {
        'f606w': 0.89682, 'f814w': 0.78386, 'f115w': 0.67045, 'f150w': 0.00143,
        'f200w': 0.00158, 'f277w': 0.00114, 'f356w': 0.57630, 'f410m': 0.00065, 'f444w': 0.00057
    },
    'nircam10': {
        'f606w': 0.52796, 'f814w': 0.56902, 'f115w': 0.37736, 'f150w': 0.32635,
        'f200w': 0.32467, 'f277w': 0.42339, 'f356w': 0.50215, 'f410m': 0.63545, 'f444w': 0.56143
    }
}
...
def setup_logging(output_dir):
    ...
    log_file = os.path.join(output_dir, "diagnostic_analysis_log_till_5f_without_f410m>5.txt")
...
def apply_f115w_selection(data_dict):
    ...
    sn_detect = (data_dict['f150w']['SNR'] > 5) & \
                ...
                (data_dict['f444w']['SNR'] > 5)     #removed (data_dict['f410m']['SNR'] > 5) & \
...
def analyze_missed_galaxies_detailed(filter_data, highlight_ids, pointing, selection_mask=None):
    ...
    for gal_id in highlight_ids:
        ...
        cut3_threshold = 0.5 + 0.44 * (color2 + 0.8) + 0.5
        color3_ok = color1 > cut3_threshold
        if not color3_ok:
            reasons.append(f"Color3 failed: color1={color1:.2f} <= {cut3_threshold:.2f}")
...
def create_diagnostic_plots(filter_data, analysis_df, highlight_ids, pointing):
    ...
    # Plot cut3 boundary
    color2_range = np.linspace(-1.5, 1.0, 100)
    cut3_line = 0.5 + 0.44 * (color2_range + 0.8) + 0.5
    plt.plot(color2_range, cut3_line, 'b--', linewidth=2, label='Cut3 boundary')
...
def create_filtered_catalogs(filter_data, selection_mask, pointing, output_base_dir):
    ...
    catalog_dir = os.path.join(output_base_dir, 'catalogue_lyman_filtered_5f', pointing)
...
def main():
    ...
    print("Applying SNR scaling factors from calibration...")
    ...
    for pointing in pointings:
        ...
        for filt in filters:
            ...
            data = read_sextractor_catalog(cat_path, pointing, filt)
            if data is None:
