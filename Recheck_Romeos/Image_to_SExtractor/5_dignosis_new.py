...
output_base_dir = './5_new_Diagnostic_Analysis'
...
def analyze_missed_galaxies_detailed(filter_data, highlight_ids, pointing, selection_mask=None):
    ...
    print(f"\n{'='*60}")
    print(f"DETAILED DIAGNOSTIC ANALYSIS FOR {pointing.upper()} (NO CUT3)")
    print(f"{'='*60}")
...
def main():
    ...
    print(f"Starting detailed diagnostic analysis of {len(pointings)} pointings (NO CUT3)")
    ...
    for pointing in pointings:
        print(f"\n{'='*80}")
        print(f"ANALYZING {pointing.upper()} (NO CUT3)")
        print(f"{'='*80}")
