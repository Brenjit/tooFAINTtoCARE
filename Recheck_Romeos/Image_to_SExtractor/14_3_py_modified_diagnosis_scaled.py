...
# Redshift bins to process
redshift_bins = ['redshift_z7-8', 'redshift_z8-10', 'redshift_z10-15']
...
def parse_scaling_factors(file_path):
    ...
    try:
        ...
        current_redshift_bin = None
        
        for line in lines:
            line = line.strip()
            
            # Check for pointing and redshift bin line
            if line.startswith('Pointing:'):
                parts = line.split(',')
                if len(parts) >= 2:
                    pointing_part = parts[0].split(':')[1].strip()
                    redshift_part = parts[1].split(':')[1].strip()
                    
                    current_pointing = pointing_part
                    current_redshift_bin = redshift_part
                    
                    if current_pointing not in scaling_factors:
                        scaling_factors[current_pointing] = {}
                    if current_redshift_bin not in scaling_factors[current_pointing]:
                        scaling_factors[current_pointing][current_redshift_bin] = {}
            
            # Check for filter scaling factors
            elif line.startswith('F') and ':' in line:
                if current_pointing and current_redshift_bin:
                    filter_part, value_part = line.split(':', 1)
                    filter_name = filter_part.strip().lower()
                    try:
                        scaling_value = float(value_part.strip())
                        scaling_factors[current_pointing][current_redshift_bin][filter_name] = scaling_value
                    except ValueError:
            ...
            elif line.startswith('Average') and current_pointing and current_redshift_bin:
                if 'Average' not in scaling_factors[current_pointing][current_redshift_bin]:
                    try:
                        avg_value = float(line.split(':')[1].strip())
                        scaling_factors[current_pointing][current_redshift_bin]['average'] = avg_value
                    except ValueError:
                        pass
...
def get_scaling_factor(scaling_factors, pointing, redshift_bin, filter_name):
    """Get the scaling factor for a specific pointing, redshift bin, and filter."""
    try:
        if (pointing in scaling_factors and 
            redshift_bin in scaling_factors[pointing] and 
            filter_name in scaling_factors[pointing][redshift_bin]):
            return scaling_factors[pointing][redshift_bin][filter_name]
        else:
            # Fallback: use average for the pointing and redshift bin
            if (pointing in scaling_factors and 
                redshift_bin in scaling_factors[pointing] and 
                'average' in scaling_factors[pointing][redshift_bin]):
                print(f"Using average scaling factor for {pointing} {redshift_bin} {filter_name}")
                return scaling_factors[pointing][redshift_bin]['average']
            else:
                # Final fallback: use 1.0 (no scaling)
                print(f"Warning: No scaling factor found for {pointing} {redshift_bin} {filter_name}, using 1.0")
                return 1.0
    except Exception as e:
        print(f"Error getting scaling factor for {pointing} {redshift_bin} {filter_name}: {e}")
        return 1.0

def setup_logging(output_dir, redshift_bin):
    """Set up logging to both console and file for specific redshift bin."""
    log_file = os.path.join(output_dir, f"diagnostic_analysis_{redshift_bin}_SNR_gt_4_veto_lst_4.txt")
    ...
    print(f"\n{'='*80}")
    print(f"DIAGNOSTIC ANALYSIS LOG - {redshift_bin} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*80}\n")

def read_sextractor_catalog(filepath, pointing, filter_name, redshift_bin, scaling_factors):
    ...
    try:
        ...
        scale_factor = get_scaling_factor(scaling_factors, pointing, redshift_bin, filter_name)
        data['SNR'] = original_snr * scale_factor
        
        print(f"Applied scaling factor {scale_factor:.6f} to {filter_name} SNR for {pointing} {redshift_bin}")
        return data
    except Exception as e:
...
def plot_snr_values(filter_data, pointing, highlight_ids, output_dir, redshift_bin, selection_mask=None):
    ...
    for filt in filters:
        ...
        plt.title(f'{pointing} {redshift_bin}: {filt} SNR vs Source ID\n(Brenjit_ID: red=normal, red+green=meets criteria)')
        ...
        # Save plot
        plot_path = os.path.join(output_dir, f'{pointing}_{redshift_bin}_{filt}_snr_with_selection.png')
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        plt.close()
    
    return total_brenjit, recovered_brenjit

def analyze_missed_galaxies_detailed(filter_data, highlight_ids, pointing, redshift_bin, selection_mask=None):
    """Detailed analysis of why specific galaxies are being missed."""
    
    print(f"\n{'='*60}")
    print(f"DETAILED DIAGNOSTIC ANALYSIS FOR {pointing.upper()} {redshift_bin}")
    print(f"{'='*60}")
    ...
    print(f"\nSUMMARY FOR {pointing} {redshift_bin}:")
    ...
    create_diagnostic_plots(filter_data, df, highlight_ids, pointing, redshift_bin)
    
    return df

def create_diagnostic_plots(filter_data, analysis_df, highlight_ids, pointing, redshift_bin):
    """Create diagnostic plots for visual analysis."""
    
    output_dir = os.path.join(output_base_dir, pointing, redshift_bin, 'diagnostics')
    os.makedirs(output_dir, exist_ok=True)
    ...
    plt.title(f'{pointing} {redshift_bin}: SNR in Detection Bands for Brenjit Sources')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{pointing}_{redshift_bin}_detection_snr.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def generate_summary_report(all_analysis_results, redshift_bin):
    """Generate a comprehensive summary report across all pointings for a specific redshift bin."""
    
    summary_file = os.path.join(output_base_dir, f"diagnostic_summary_report_{redshift_bin}.txt")
    
    with open(summary_file, 'w') as f:
        f.write(f"COMPREHENSIVE DIAGNOSTIC SUMMARY REPORT - {redshift_bin}\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write(f"\nOVERALL SUMMARY FOR {redshift_bin}:\n")
        f.write("=" * 30 + "\n")
...
def create_filtered_catalogs(filter_data, selection_mask, pointing, redshift_bin, output_base_dir):
    """Create filtered catalogs for sources that meet the selection criteria."""
    catalog_dir = os.path.join(output_base_dir, 'catalogue_lyman_filtered_final', pointing, redshift_bin)
    os.makedirs(catalog_dir, exist_ok=True)
    
    print(f"Creating filtered catalogs for {pointing} {redshift_bin} in {catalog_dir}")
...
def generate_final_summary(all_analysis_results, redshift_bin):
    """Generate a final summary table with counts and percentages and detailed criteria failure information."""
    print(f"\n{'='*80}")
    print(f"FINAL SUMMARY TABLE - {redshift_bin}")
    print(f"{'='*80}")
...
def main():
    """Main function with detailed diagnostic analysis for multiple redshift bins."""
    ...
    # Process each redshift bin separately
    for redshift_bin in redshift_bins:
        print(f"\n{'='*80}")
        print(f"PROCESSING REDSHIFT BIN: {redshift_bin}")
        print(f"{'='*80}")
        
        # Use the single output directory defined at the top
        os.makedirs(output_base_dir, exist_ok=True)
        
        # Setup logging for this redshift bin
        setup_logging(output_base_dir, redshift_bin)
        
        print(f"Starting detailed diagnostic analysis of {len(pointings)} pointings for {redshift_bin}")
        ...
        for pointing in pointings:
            print(f"\n{'='*80}")
            print(f"ANALYZING {pointing.upper()} - {redshift_bin}")
            print(f"{'='*80}")
            ...
            for filt in filters:
                ...
                data = read_sextractor_catalog(cat_path, pointing, filt, redshift_bin, scaling_factors)
                if data is None:
            ...
            if not missing_data:
                ...
                create_filtered_catalogs(filter_data, selection_mask, pointing, redshift_bin, output_base_dir)
                ...
                total, recovered = plot_snr_values(
                    ...
                    pointing, 
                    highlight_ids.get(pointing, []),
                    os.path.join(output_base_dir, pointing, redshift_bin),
                    redshift_bin,
                    selection_mask
                )
        
        # Generate summary report across all pointings for this redshift bin
        generate_summary_report(all_analysis_results, redshift_bin)
        
        # Generate final summary table for this redshift bin
        generate_final_summary(all_analysis_results, redshift_bin)
        
        print(f"\nDiagnostic analysis for {redshift_bin} complete. All outputs saved to: {output_base_dir}")
