...
output_dir = './14_SNR_Comparison_Analysis_NMAD'

# Define redshift bins and pointings
...
def create_comparison_catalog(sextractor_data, nmad_data, nmad_snr, pointing, redshift_bin, output_dir):
    """Create a comprehensive catalog comparing both SNR calculation methods."""
    os.makedirs(output_dir, exist_ok=True)
    
    # Get all source IDs from NMAD data for this pointing and redshift bin
    source_ids = get_source_ids_from_nmad(nmad_data)
    
    if not source_ids:
        print(f"No source IDs found in NMAD data for {pointing} {redshift_bin}")
        return None, None
    
    print(f"Found {len(source_ids)} sources in NMAD data for {pointing} {redshift_bin}")
    ...
    if not output_data:
        print(f"No valid comparison data for {pointing} {redshift_bin}")
        return None, None
        
    df = pd.DataFrame(output_data)
    output_file = os.path.join(output_dir, f'{pointing}_{redshift_bin}_snr_comparison.csv')
    ...
    summary_file = os.path.join(output_dir, f'{pointing}_{redshift_bin}_snr_summary.csv')
...
def plot_individual_source_comparison(comparison_data, pointing, redshift_bin, output_dir):
    """Generate individual plots for each source showing SExtractor vs NMAD SNR across filters."""
    individual_dir = os.path.join(output_dir, pointing, redshift_bin, 'individual_sources')
    ...
    for src_id in source_ids:
        ...
        plt.ylabel('NMAD SNR')
        plt.title(f'{pointing} {redshift_bin} - Source {src_id}: SExtractor vs NMAD SNR by Filter')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        plot_filename = os.path.join(individual_dir, f'{pointing}_{redshift_bin}_source_{src_id}_snr_comparison.png')
        plt.savefig(plot_filename, dpi=150, bbox_inches='tight')
        plt.close()

def plot_all_sources_comparison(comparison_data, pointing, redshift_bin, output_dir):
    """Generate plots showing all sources with different colors."""
    all_sources_dir = os.path.join(output_dir, pointing, redshift_bin, 'all_sources_comparison')
    ...
    plt.xlabel('SExtractor SNR')
    plt.ylabel('NMAD SNR')
    plt.title(f'{pointing} {redshift_bin}: All Sources - SExtractor vs NMAD SNR')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(all_sources_dir, f'{pointing}_{redshift_bin}_all_sources_comparison.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    for filt in filters:
        ...
        plt.xlabel('SExtractor SNR')
        plt.ylabel('NMAD SNR')
        plt.title(f'{pointing} {redshift_bin}: {filt.upper()} - SExtractor vs NMAD SNR by Source')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(all_sources_dir, f'{pointing}_{redshift_bin}_{filt}_sources_comparison.png'), dpi=150, bbox_inches='tight')
        plt.close()

def plot_snr_comparison(comparison_data, pointing, redshift_bin, output_dir):
    """Generate plots comparing SNR values from both methods."""
    if comparison_data is None or len(comparison_data) == 0:
        print(f"No comparison data for {pointing} {redshift_bin}")
        return
    
    plot_individual_source_comparison(comparison_data, pointing, redshift_bin, output_dir)
    plot_all_sources_comparison(comparison_data, pointing, redshift_bin, output_dir)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, pointing, redshift_bin, f'{pointing}_{redshift_bin}_snr_comparison_by_filter.png'), dpi=150, bbox_inches='tight')
    plt.close()

def save_fit_summary(all_comparison_data, output_dir, sigma_clip=3.0):
    """
    Save a text file with pointing, redshift bin, filter, 10^intercept values, and average per pointing.
    Uses sigma-clipping to remove high-end deviations before averaging.
    """
    summary_lines = []
    
    for (pointing, redshift_bin), comp_data in all_comparison_data.items():
        fit_values = []
        summary_lines.append(f"\nPointing: {pointing}, Redshift Bin: {redshift_bin}")
...
def main():
    ...
    for pointing in pointings:
        ...
        for redshift_bin in redshift_bins:
            print(f"  Redshift bin: {redshift_bin}")
            ...
            # Read NMAD catalog
            nmad_file = os.path.join(nmad_base_dir, redshift_bin, pointing, 'catalogs', f'{pointing}_{redshift_bin.split("_")[1]}_nmad_catalogue.cat')
            ...
            if nmad_data is None or len(nmad_data) == 0:
                print(f"    Could not read NMAD data for {pointing} {redshift_bin}")
                continue
            ...
            comparison_df, summary_df = create_comparison_catalog(
                sextractor_data, nmad_data, nmad_snr, pointing, redshift_bin, output_dir
            )
            ...
            if comparison_df is not None:
                ...
                # Create plots
                plot_dir = os.path.join(output_dir, pointing, redshift_bin)
                os.makedirs(plot_dir, exist_ok=True)
                plot_snr_comparison(comparison_df, pointing, redshift_bin, output_dir)
                
                print(f"    Processed {len(comparison_df['NUMBER'].unique())} sources for {pointing} {redshift_bin}")
    
    if all_comparison_data:
        # Combine all data
        all_data_list = []
        for key, data in all_comparison_data.items():
            data['pointing'] = key[0]
            data['redshift_bin'] = key[1]
        ...
        overall_summary = all_data.groupby(['FILTER', 'redshift_bin']).agg({
        ...
        print(f"\nAnalysis complete. Processed {len(all_data)} measurements across {len(all_comparison_data)} pointing/redshift combinations")
    else:
