...
def apply_f814w_selection(data_dict, brenjit_ids):
    """Applies F814W-dropout (z=5.5-8.5) selection criteria."""
    ...
    # Only keep sources that are in our matched catalog
    mask = mask & np.isin(data_dict['f115w']['NUMBER'], brenjit_ids)
    
    print(f"Found {np.sum(mask)} candidates.")
    return mask

def apply_f115w_selection(data_dict, brenjit_ids):
    """Applies F115W-dropout (z=8.5-12) selection criteria."""
    ...
    # Only keep sources that are in our matched catalog
    mask = mask & np.isin(data_dict['f150w']['NUMBER'], brenjit_ids)
    
    print(f"Found {np.sum(mask)} candidates.")
    return mask
    
def apply_f150w_selection(data_dict, brenjit_ids):
    """Applies F150W-dropout (z=12-15) selection criteria."""
    ...
    # Only keep sources that are in our matched catalog
    mask = mask & np.isin(data_dict['f277w']['NUMBER'], brenjit_ids)
    
    print(f"Found {np.sum(mask)} candidates.")
    return mask

def main():
    """Main function with new requirements."""
    ...
    # Create a dictionary to store which sources passed which criteria
    selection_log = {row['Brenjit_ID']: [] for _, row in matched_cat.iterrows()}
    ...
    for pointing in pointings:
        ...
        # Get Brenjit IDs for this pointing
        pointing_ids = matched_cat[matched_cat['POINTING'] == pointing]['Brenjit_ID'].values
        if len(pointing_ids) == 0:
            print(f"No matched sources for pointing {pointing}, skipping.")
            continue
        ...
        for z_range, selection_func in selections.items():
            ...
            candidates_mask = selection_func(filter_data, pointing_ids)
            
            # Get the Brenjit IDs of the selected sources
            selected_ids = filter_data['f115w']['NUMBER'][candidates_mask].data if 'f115w' in filter_data else \
                          filter_data['f150w']['NUMBER'][candidates_mask].data if 'f150w' in filter_data else \
                          filter_data['f277w']['NUMBER'][candidates_mask].data
            
            # Update selection log
            for source_id in selected_ids:
                if source_id in selection_log:
                    selection_log[source_id].append(z_range)
    ...
    # Save the selection log
    log_filename = os.path.join(output_base_dir, 'selection_log.txt')
    with open(log_filename, 'w') as f:
        f.write("Brenjit_ID\tRedshift_Range(s)\n")
        for source_id, z_ranges in selection_log.items():
            if z_ranges:
                f.write(f"{source_id}\t{', '.join(z_ranges)}\n")
            else:
                f.write(f"{source_id}\tNone\n")

    print("\n--- Processing Complete ---")
    print(f"Output saved in the '{output_base_dir}' directory, structured by redshift and pointing.")
    print(f"Selection log saved to {log_filename}")
